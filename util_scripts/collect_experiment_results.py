...
def main(controller_output: str,
    ...
    # collect results per plant
    ctrl_output = Path(controller_output).resolve()
    base_output_dir = Path(target_dir).resolve()

    for i in range(num_plants):
        print(f'Collecting results for setup {i:d} out of {num_plants:d}')

        # set up target dir
        out_dir = base_output_dir / f'plant_{i:02d}'
        out_dir.mkdir(parents=True, exist_ok=False)

        # collect controller results
        for ctrl_file in (ctrl_output / f'controller_{i:02d}').iterdir():
        ...
        subprocess.run(
            ['sshpass', '-p', rpi_passwd,
             ...
             f'"{plant_output}/plant_{i:02d}/*"', out_dir]
        )
    print('Done')
